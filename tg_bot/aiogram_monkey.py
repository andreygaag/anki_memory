...
class MyExecutor(executor.Executor):
    async def start_polling(self, reset_webhook=None, timeout=20, relax=0.1, fast=True, allowed_updates: Optional[list[str]] = None):
            """
            ...
            if loop.is_running():
                try:
                    ...
                    asyncio.create_task(
                        self.dispatcher.start_polling(reset_webhook=reset_webhook, timeout=timeout, relax=relax, fast=fast, allowed_updates=allowed_updates)
                    )
                    await Future()
...
async def start_polling(dispatcher, *, loop=None, skip_updates=False, reset_webhook=True,
